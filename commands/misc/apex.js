  ...
  execute: async (bot, message, args) => {
    ...
    if (!args[1])
      return message.channel.send(
        'Please supply a platform to check. `pc`, `xbox` or `ps4`'
      );
    ...
    try {
      const results = await ApexTab.searchPlayer(
        args[0],
        platform ? platform : API.Platform.PC
      );
        ...
        const embed = new MessageEmbed()
          ...
          .setDescription(
            `
                                ...
                                **Visits:** ${visits || 0}
                                **PlayTime:** ${
                                  Math.ceil(utime / (1000 * 60 * 60 * 24)) || 0
                                } days
                            `
          )
          .setTimestamp();
      ...
      console.log(err);
      return message.channel.send("Can't find a player");
    }
  },
};
